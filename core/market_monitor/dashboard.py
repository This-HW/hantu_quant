...
class ChartGenerator:
    ...
    def create_market_overview_chart(
        ...
        try:
            ...
            fig.add_trace(
                go.Scatter(
                    x=timestamps, y=kospi_values, name="KOSPI", line=dict(color="blue")
                ),
                row=1,
                col=1,
            )
            ...
            fig.add_trace(
                go.Scatter(
                    x=timestamps, y=kosdaq_values, name="KOSDAQ", line=dict(color="red")
                ),
                row=1,
                col=2,
            )
            ...
            # 거래대금
            trading_values = [
                s.total_trading_value / 1e12 for s in snapshots
            ]  # 조원 단위
            fig.add_trace(
                go.Scatter(
                    x=timestamps,
            ...
            # 레이아웃 설정
            fig.update_layout(
                title="시장 개요", height=self._config.chart_height * 2, showlegend=True
            )
            ...
            for i in range(grid_size):
                ...
                for j in range(grid_size):
                    ...
                    if idx < len(stocks):
                        ...
                        text_row.append(
                            f"{stock.stock_name}<br>{stock.price_change_rate:.1%}"
                        )
                    else:
                        z_row.append(0)
                        text_row.append("")
    ...
    def create_volume_analysis_chart(
        ...
        try:
            ...
            stock_names = [
                s.stock_name[:8] for s in high_volume_stocks
            ]  # 이름 길이 제한
            ...
            fig.add_trace(
                go.Scatter(
                    ...
                    name="종목별 위치",
                    marker=dict(
                        size=8, color=price_changes, colorscale="RdYlGn", showscale=True
                    ),
                ),
                row=1,
                col=2,
            ...
            # 레이아웃 설정
            fig.update_layout(
                title="거래량 분석", height=self._config.chart_height, showlegend=True
            )
    ...
    def create_alert_timeline(self, alerts: List[AnomalyAlert]) -> Optional[str]:
        ...
        try:
            ...
            [alert.severity.value for alert in recent_alerts]
            [alert.title for alert in recent_alerts]
            [color_map.get(alert.severity, "gray") for alert in recent_alerts]
    ...
    def create_market_status_gauge(
        self, current_snapshot: MarketSnapshot
    ) -> Optional[str]:
        """시장 상태 게이지 생성"""
        try:
        ...
        try:
            ...
            if self._config.chart_format == "html":
                filepath = os.path.join(
                    self._config.output_dir, f"{full_filename}.html"
                )
    ...
    def collect_metrics(
        ...
        try:
            ...
            metrics = DashboardMetrics(
                ...
                critical_alerts=len(
                    [a for a in recent_alerts if a.severity == AnomalySeverity.CRITICAL]
                ),
                high_alerts=len(
                    [a for a in recent_alerts if a.severity == AnomalySeverity.HIGH]
                ),
...
class MonitoringDashboard:
    ...
    def __init__(
        self, config: DashboardConfig = None, data_dir: str = "data/dashboard"
    ):
        """
        초기화
    ...
    def _update_charts(self):
        ...
        try:
            ...
            # 최근 스냅샷들 조회
            recent_snapshots = self._market_monitor.get_recent_snapshots(
                self._config.historical_hours
            )
            ...
            # 1. 시장 개요
            if recent_snapshots:
                chart_path = self._chart_generator.create_market_overview_chart(
                    recent_snapshots
                )
                if chart_path:
            ...
            # 2. 변동성 히트맵
            chart_path = self._chart_generator.create_volatility_heatmap(
                current_snapshot
            )
            if chart_path:
            ...
            # 3. 거래량 분석
            chart_path = self._chart_generator.create_volume_analysis_chart(
                current_snapshot
            )
            if chart_path:
            ...
            # 5. 시장 상태 게이지
            chart_path = self._chart_generator.create_market_status_gauge(
                current_snapshot
            )
            if chart_path:
    ...
    def get_dashboard_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "last_update": (
                self._current_metrics.timestamp.isoformat()
