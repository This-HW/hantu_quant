...
class DependencyResolver:
    ...
    def __init__(
        self, repository: IPackageRepository, tracker: InstalledPackageTracker
    ):
        """초기화"""
...
class PackageInstaller(IPackageInstaller):
    ...
    def install_package(
        self, name: str, version: Optional[SemanticVersion] = None, force: bool = False
    ) -> bool:
        """패키지 설치"""
        with self._lock:
            try:
    ...
    def _install_single_package(self, package_info: IPackageInfo) -> None:
        """단일 패키지 설치"""
        # 패키지 파일 조회
        package_path = self.repository.get_package(
    ...
    def _register_entry_points(self, package_info: IPackageInfo) -> None:
        ...
        try:
            ...
            from ..registry.interfaces import ModuleType, ModuleStatus  # noqa: F401
            ...
            module_metadata = ModuleMetadata(
                ...
                entry_points={
                    ep.name: ep.get_import_path() for ep in package_info.entry_points
                },
            )
    ...
    def uninstall_package(self, name: str) -> bool:
        ...
        with self._lock:
            try:
                ...
                if isinstance(
                    e, (PackageNotInstalledError, PackageUninstallationError)
                ):
                    raise
    ...
    def update_package(
        self, name: str, target_version: Optional[SemanticVersion] = None
    ) -> bool:
        """패키지 업데이트"""
        with self._lock:
            try:
                ...
                raise PackageUpdateError(
                    name,
                    "unknown",
                    str(target_version) if target_version else "unknown",
                    f"Update failed: {e}",
                    e,
                )
