    ...
    def detect_candlestick_patterns(self, p_ohlc_data):
        ...
        try:
            ...
            for i in range(1, len(p_ohlc_data)):
                ...
                _v_lower_shadow = (
                    _v_curr["open"] - _v_curr["low"]
                    if _v_curr["close"] > _v_curr["open"]
                    else _v_curr["close"] - _v_curr["low"]
                )

                if (
                    _v_total_range > 0
                    and _v_body / _v_total_range < 0.3
                    and _v_lower_shadow / _v_total_range > 0.6
                ):
            ...
            if len(p_ohlc_data) >= 2:
                ...
                if (
                    _v_prev["close"] < _v_prev["open"]
                    and _v_last["close"] > _v_last["open"]  # 이전 캔들이 음봉
                    and _v_last["open"] < _v_prev["close"]  # 현재 캔들이 양봉
                    and _v_last["close"]  # 현재 시가가 이전 종가보다 낮음
                    > _v_prev["open"]
                ):  # 현재 종가가 이전 시가보다 높음
...
class VolumeAnalysis:
    ...
    def analyze_volume_pattern(self, p_volumes, p_prices):
        ...
        try:
            ...
            _v_older_avg = (
                sum(p_volumes[:-3]) / len(p_volumes[:-3])
                if len(p_volumes) > 3
                else _v_recent_avg
            )
            ...
            _v_strength = min(
                100.0,
                (_v_recent_avg / _v_older_avg * 50.0) if _v_older_avg > 0 else 50.0,
            )
...
class PriceAttractivenessLegacy:
    ...
    def to_price_attractiveness(self) -> PriceAttractiveness:
        ...
        return PriceAttractiveness(
            ...
            analysis_date=(
                datetime.fromisoformat(self.analysis_date)
                if isinstance(self.analysis_date, str)
                else self.analysis_date
            ),
...
class TechnicalIndicators:
    ...
    def calculate_bollinger_bands(
        p_prices: List[float], p_period: int = 20, p_std_dev: float = 2.0
    ...
    @staticmethod
    def calculate_macd(
        p_prices: List[float], p_fast: int = 12, p_slow: int = 26, p_signal: int = 9
    ) -> Tuple[float, float, float]:
        """MACD 계산"""
        try:
            ...
            if len(_v_prices) >= p_slow + p_signal:
                ...
                _v_signal_line = TechnicalIndicators._calculate_ema(
                    _v_macd_values, p_signal
                )[-1]
            else:
        ...
        try:
            ...
            for i in range(p_k_period - 1, len(_v_closes)):
                ...
                if _v_highest_high == _v_lowest_low:
                    ...
                    _v_k = (
                        (_v_closes[i] - _v_lowest_low)
                        / (_v_highest_high - _v_lowest_low)
                    ) * 100
...
class PriceAnalyzer(IPriceAnalyzer):
    ...
    @inject
    def __init__(
        self, p_config_file: str = "core/config/api_config.py", config=None, logger=None
    ):
        """초기화 메서드"""
    ...
    def analyze_multiple_stocks(
        ...
        for _v_stock_data in p_stock_list:
            try:
                ...
                self._logger.error(
                    f"종목 {_v_stock_data.get('stock_code', 'Unknown')} 분석 오류: {e}",
                    exc_info=True,
                )
                continue
    ...
    ) -> Tuple[float, List[TechnicalSignal]]:
        """기술적 지표 분석 (새 인터페이스 구현)"""
        try:
            _v_score, _v_legacy_signals = self._analyze_technical_indicators(
                p_stock_data
            )
    ...
    def _analyze_price_attractiveness_legacy(
        self, p_stock_data: Dict
    ) -> PriceAttractivenessLegacy:
        ...
        try:
            ...
            if isinstance(_v_current_price_raw, list):
                _v_current_price = (
                    float(_v_current_price_raw[-1]) if _v_current_price_raw else 0.0
                )
            else:
                _v_current_price = (
                    float(_v_current_price_raw) if _v_current_price_raw else 0.0
                )
            ...
            # 각 분석 영역별 점수 계산
            _v_technical_score, _v_technical_signals = (
                self._analyze_technical_indicators(p_stock_data)
            )
            _v_volume_score = self._analyze_volume(p_stock_data)
            _v_pattern_score = self._analyze_patterns(p_stock_data)

            # 기울기 지표 분석 추가
            _v_slope_score, _v_slope_signals = self._analyze_slope_indicators(
                p_stock_data
            )
            ...
            # 종합 점수 계산 (가중 평균)
            _v_total_score = (
                _v_technical_score * 0.40
                + _v_volume_score * 0.30  # 기술적 분석 40%
                + _v_pattern_score * 0.20  # 거래량 분석 30%
                + _v_slope_score * 0.10  # 패턴 분석 20%  # 기울기 분석 10%
            )

            # 진입가, 목표가, 손절가 계산
            _v_entry_price, _v_target_price, _v_stop_loss = self._calculate_target_stop(
                _v_current_price, _v_total_score
            )
            _v_expected_return = (
                ((_v_target_price - _v_entry_price) / _v_entry_price) * 100
                if _v_entry_price > 0
                else 0.0
            )
            ...
            _v_confidence = self._calculate_confidence(
                _v_technical_signals, _v_total_score
            )

            # 선정 이유 및 시장 상황
            _v_selection_reason = self._generate_selection_reason(
                _v_technical_signals, _v_total_score
            )
    ...
    def _create_default_result_legacy(
        self, p_stock_data: Dict
    ) -> PriceAttractivenessLegacy:
        """기본 결과 생성 (기존 호환성용)"""
        return PriceAttractivenessLegacy(
    ...
    def _analyze_technical_indicators(
        self, p_stock_data: Dict
    ) -> Tuple[float, List[TechnicalSignalLegacy]]:
        """기술적 지표 분석 (기존 로직)"""
        _v_signals = []
        _v_scores = []

        # 안전하게 현재가 추출
        _v_current_price = self._safe_get_float(
        ...
        _v_bb_upper, _v_bb_middle, _v_bb_lower = (
            TechnicalIndicators.calculate_bollinger_bands(_v_prices)
        )

        if _v_current_price <= _v_bb_lower * 1.02:  # 하단 밴드 근처
            _v_signals.append(
                TechnicalSignalLegacy(
        ...
        # 스토캐스틱 분석 (실제 계산)
        _v_k, _v_d = TechnicalIndicators.calculate_stochastic(
            _v_highs, _v_lows, _v_prices
        )
        ...
        try:
            ...
            if (recent_bars is None) or (
                isinstance(recent_bars, list) and len(recent_bars) == 0
            ):
                # 필요 시 API로 보충
                try:
            ...
            if df_mb is not None:
                ...
                closes = _pd.to_numeric(
                    df_mb.get("close", df_mb.get("stck_prpr", [])), errors="coerce"
                )
                vols = _pd.to_numeric(
                    df_mb.get("volume", df_mb.get("acml_vol", [])), errors="coerce"
                )
                if (
                    len(closes) > 0
                    and len(vols) > 0
                    and closes.notna().any()
                    and vols.notna().any()
                ):
    ...
    def _analyze_volume(self, p_stock_data: Dict) -> float:
        ...
        try:
            ...
            _v_volumes = self._safe_get_list(
                p_stock_data.get("recent_volumes", []), p_default_size=20
            )
    ...
    def _analyze_slope_indicators(
        ...
        try:
            ...
            _v_recent_close = self._safe_get_list(
                p_stock_data.get("recent_close_prices", []), p_default_size=60
            )
            _v_recent_vol = self._safe_get_list(
                p_stock_data.get("recent_volumes", []),
                p_default_size=len(_v_recent_close) or 60,
            )
            if _v_recent_close and len(_v_recent_close) >= 60:
                dates = pd.date_range(
                    end=datetime.now().date(), periods=len(_v_recent_close), freq="D"
                )
    ...
    def _calculate_target_stop(
        self, p_entry_price: float, p_score: float
    ) -> Tuple[float, float, float]:
        """진입가, 목표가, 손절가 계산"""
        # 점수에 따른 목표 수익률 조정
    ...
    def _calculate_historical_volatility(
        self, p_prices: List[float], p_period: int = 20
    ) -> float:
        """
        역사적 변동성 계산 (연율화)
    ...
    def _calculate_confidence(
        self, p_signals: List[TechnicalSignal], p_score: float
    ) -> float:
        """신뢰도 계산"""
        if not p_signals:
            return 0.3
    ...
    def save_analysis_results(
        self, p_results: List[PriceAttractiveness], p_file_path: str = None
    ) -> bool:
        """분석 결과 저장"""
        try:
            if not p_file_path:
            ...
            _v_save_data = {
                ...
                "metadata": {"analyzer_version": "1.0.0", "weights": self._v_weights},
            }
