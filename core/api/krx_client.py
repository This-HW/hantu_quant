...
try:
    from pykrx.website.krx.market.core import 상장종목검색  # noqa: F401
...
class KRXClient:
    ...
    def save_stock_list(self) -> None:
        ...
        try:
            ...
            logger.info(
                f"[save_stock_list] 주식 목록 저장 완료 - 파일: {file_path}, 종목 수: {len(df)}개"
            )
            ...
            logger.error(
                f"[save_stock_list] 주식 목록 저장 중 오류 발생: {str(e)}",
                exc_info=True,
            )
            logger.error(
                f"[save_stock_list] 상세 에러: {e.__class__.__name__}", exc_info=True
            )
            raise
    ...
    def get_stock_list(self, market: str = "ALL") -> pd.DataFrame:
        ...
        try:
            ...
            logger.error(
                f"[get_stock_list] 종목 목록 조회 중 오류 발생: {str(e)}", exc_info=True
            )
            logger.error(
                f"[get_stock_list] 상세 에러: {e.__class__.__name__}", exc_info=True
            )
            raise
    ...
    def get_stock_info(self, ticker: str) -> dict:
        ...
        try:
            ...
            logger.error(
                f"[get_stock_info] 종목 정보 조회 중 오류 발생: {str(e)}", exc_info=True
            )
            logger.error(
                f"[get_stock_info] 상세 에러: {e.__class__.__name__}", exc_info=True
            )
            raise
    ...
    def collect_fundamentals_via_kis(
        ...
        try:
            ...
            logger.info(
                f"[collect_fundamentals_via_kis] KIS API로 {len(stock_codes)}개 종목 수집 시작"
            )
            ...
            for i in range(0, total, batch_size):
                ...
                logger.info(
                    f"[collect_fundamentals_via_kis] 배치 {batch_num}/{total_batches} 처리 중..."
                )
            ...
            logger.error(
                f"[collect_fundamentals_via_kis] 배치 수집 오류: {e}", exc_info=True
            )
            return pd.DataFrame()

    def _get_valid_trading_date(
        self, target_date: str, max_days_back: int = 14
    ) -> Optional[str]:
        """유효한 거래일 찾기 (주말/휴일 고려)
            ...
            for i in range(max_days_back):
                ...
                try:
                    ...
                    if tickers and len(tickers) > 0:
                        logger.debug(
                            f"[_get_valid_trading_date] 유효한 거래일 발견: {check_date}"
                        )
                        return check_date
                except Exception:
                    continue
    ...
    def get_market_fundamentals(
        ...
        if source == "kis":
            ...
            if not df.empty:
                ...
                logger.info(
                    f"[get_market_fundamentals] KIS API 성공 - {len(df)}개 종목"
                )
                return df
            else:
    ...
    def _get_market_fundamentals_pykrx(
        self, date: Optional[str] = None, market: str = "ALL"
    ) -> pd.DataFrame:
        ...
        try:
            ...
            if valid_date:
                date = valid_date
                logger.info(
                    f"[_get_market_fundamentals_pykrx] 유효한 거래일 사용: {date}"
                )
            ...
            for i in range(14):  # 최대 2주 전까지 시도
                ...
                try:
                    ...
                    if not df.empty and len(df.columns) > 0:
                        if i > 0:
                            logger.info(
                                f"[_get_market_fundamentals_pykrx] {date} 실패, {try_date} 사용"
                            )
                        break
                except Exception as e:
                    last_error = e
                    logger.debug(
                        f"[_get_market_fundamentals_pykrx] {try_date} 조회 실패: {e}"
                    )
                    continue
            ...
            if df.empty:
                if last_error:
                    logger.warning(
                        f"[_get_market_fundamentals_pykrx] 재무 데이터 조회 실패: {last_error}"
                    )
                else:
                    logger.warning(
            ...
            if "EPS" in df.columns and "BPS" in df.columns:
                df["ROE"] = df.apply(
                    lambda row: (
                        (row["EPS"] / row["BPS"] * 100) if row["BPS"] > 0 else 0.0
                    ),
                    axis=1,
                )
            else:
            ...
            logger.info(
                f"[_get_market_fundamentals_pykrx] 재무 데이터 조회 성공 - {len(df)}개 종목"
            )
            return df

        except Exception as e:
            logger.error(
                f"[_get_market_fundamentals_pykrx] 재무 데이터 조회 오류: {e}",
                exc_info=True,
            )
            return pd.DataFrame()
    ...
    def get_stock_fundamental(
        ...
        try:
            ...
            if DEFAULT_DATA_SOURCE == "kis":
                ...
                if kis_data:
                    logger.debug(
                        f"[get_stock_fundamental] KIS API로 {ticker} 조회 성공"
                    )
                    return kis_data
            ...
            if PYKRX_AVAILABLE:
                ...
                return {
                    "ticker": ticker,
                    "per": (
                        float(row.get("PER", 0)) if pd.notna(row.get("PER")) else 0.0
                    ),
                    "pbr": (
                        float(row.get("PBR", 0)) if pd.notna(row.get("PBR")) else 0.0
                    ),
                    "eps": (
                        float(row.get("EPS", 0)) if pd.notna(row.get("EPS")) else 0.0
                    ),
                    "bps": (
                        float(row.get("BPS", 0)) if pd.notna(row.get("BPS")) else 0.0
                    ),
                    "div": (
                        float(row.get("DIV", 0)) if pd.notna(row.get("DIV")) else 0.0
                    ),
                    "dps": (
                        float(row.get("DPS", 0)) if pd.notna(row.get("DPS")) else 0.0
                    ),
                    "roe": (
                        float(row.get("ROE", 0)) if pd.notna(row.get("ROE")) else 0.0
                    ),
                }
            ...
            logger.error(
                f"[get_stock_fundamental] 종목 {ticker} 재무 데이터 조회 오류: {e}",
                exc_info=True,
            )
            return None
    ...
    def save_market_fundamentals_to_db(self, date: Optional[str] = None) -> int:
        ...
        try:
            ...
            with get_session() as session:
                for _, row in df.iterrows():
                    ...
                    existing = (
                        ...
                        .filter_by(stock_code=stock_code, date=data_date)
                        .first()
                    )
                    ...
                    if existing:
                        ...
                        existing.div = (
                            float(row.get("DIV", 0))
                            if pd.notna(row.get("DIV"))
                            else None
                        )
                        existing.dps = (
                            float(row.get("DPS", 0))
                            if pd.notna(row.get("DPS"))
                            else None
                        )
                        existing.roe = (
                            float(row.get("ROE", 0))
            ...
            logger.info(
                f"[save_market_fundamentals_to_db] DB 저장 완료 - {saved_count}개 종목"
            )
            return saved_count

        except Exception as e:
            logger.error(
                f"[save_market_fundamentals_to_db] DB 저장 오류: {e}", exc_info=True
            )
            return 0
    ...
    def load_fundamentals_from_db(
        ...
        try:
            ...
            logger.error(
                f"[load_fundamentals_from_db] DB 조회 오류: {e}", exc_info=True
            )
            return None
    ...
    def save_market_fundamentals(self, date: Optional[str] = None) -> int:
        ...
        try:
            ...
            logger.info(
                f"[save_market_fundamentals] 저장 완료 - DB: {db_count}개, 파일: {file_path}"
            )
            return db_count

        except Exception as e:
            logger.error(
                f"[save_market_fundamentals] 재무 데이터 저장 오류: {e}", exc_info=True
            )
            return 0
        ...
        try:
            ...
            with get_session() as session:
                if date:
                    data_date = datetime.strptime(date, "%Y%m%d").date()
                    results = (
                        session.query(StockFundamental).filter_by(date=data_date).all()
                    )
                else:
                    # 가장 최신 날짜 조회
                    latest = (
                        session.query(StockFundamental.date)
                ...
                if results:
                    ...
                    logger.info(
                        f"[load_market_fundamentals] DB에서 로드 성공 - {len(df)}개 종목"
                    )
                    return df

            # 2. DB에 없으면 파일에서 로드
            logger.debug(
                "[load_market_fundamentals] DB에 데이터 없음, 파일에서 로드 시도"
            )
            ...
            if date:
                ...
                logger.info(
                    f"[load_market_fundamentals] 파일에서 로드 성공 - {latest_file}"
                )
                return df

        except Exception as e:
            logger.error(
                f"[load_market_fundamentals] 재무 데이터 로드 오류: {e}", exc_info=True
            )
            return pd.DataFrame()
