...
class FeatureSelector:
    ...
    def extract_all_features(self, ohlcv_data: pd.DataFrame) -> CombinedFeatures:
        ...
        try:
            ...
            combined_features = CombinedFeatures(
                slope_features=slope_features, volume_features=volume_features
            )

            self._logger.debug(
                f"전체 피처 추출 완료: {len(combined_features.get_feature_names())}개"
            )
            ...
            return CombinedFeatures(
                slope_features=SlopeFeatures(), volume_features=VolumeFeatures()
            )

    def extract_all_features_from_stock_data(
        ...
        try:
            # 기울기 피처 추출
            slope_features = self._slope_extractor.extract_features_from_stock_data(
                stock_data
            )

            # 볼륨 피처 추출
            volume_features = self._volume_extractor.extract_features_from_stock_data(
                stock_data
            )
    ...
    def analyze_feature_importance(
        self, features_list: List[CombinedFeatures], targets: List[float]
        ...
        try:
            ...
            self._volume_extractor.get_feature_names()
            ...
            self._logger.info(
                f"피처 중요도 분석 완료: {len(feature_importances)}개 피처"
            )
    ...
    def select_optimal_features(
        ...
        try:
            ...
            self._logger.info(
                f"피처 선택 완료: {len(selected_features)}/{len(feature_names)} 피처 선택"
            )
            return result
    ...
    def _select_features_by_importance_and_correlation(
        self,
        feature_importances: List[FeatureImportance],
        feature_correlations: Dict[str, float],
        max_features: int,
    ...
    def save_selection_result(
        self, result: FeatureSelectionResult, filepath: str
    ) -> bool:
        """피처 선택 결과를 파일에 저장"""
        try:
