...
class StrategyReporter:
    ...
    def record_daily_strategy_return(self, strategy_name: str, date: str,
    ...
    def _calculate_strategy_performance(self, strategy_name: str, 
        ...
        try:
            with sqlite3.connect(self._db_path) as conn:
                ...
                df = pd.read_sql_query(query, conn, params=[strategy_name, start_date, end_date])
                ...
                # 평균 수익/손실
                avg_win = daily_returns[daily_returns > 0].mean() * 100 if (daily_returns > 0).any() else 0
                avg_loss = daily_returns[daily_returns < 0].mean() * 100 if (daily_returns < 0).any() else 0
                ...
                market_correlation = np.corrcoef(daily_returns, daily_returns - active_returns)[0, 1] if len(daily_returns) > 1 else 0
                
                if len(active_returns) > 1:
                    benchmark_returns = daily_returns - active_returns
                    beta = np.cov(daily_returns, benchmark_returns)[0, 1] / np.var(benchmark_returns) if np.var(benchmark_returns) > 0 else 0
                    alpha = (annualized_return - risk_free_rate*100) - beta * (np.mean(benchmark_returns) * 252 * 100 - risk_free_rate*100)
                else:
                    beta = 0
                    alpha = 0
                ...
                monthly_data = df.set_index('date').resample('M')['daily_return'].sum().tolist()
                
                # 분기별 수익률
                quarterly_data = df.set_index('date').resample('Q')['daily_return'].sum().tolist()
    ...
    def _compare_strategies(self, period_days: int) -> StrategyComparison:
        ...
        try:
            ...
            for strategy in self._strategies:
                performance = self._calculate_strategy_performance(strategy, start_date, end_date)
                if performance:
            ...
            # 최고/최악 성과자 찾기
            returns = {name: perf.total_return for name, perf in strategy_performances.items()}
            ...
            # 리스크 조정 순위 (샤프 비율 기준)
            sharpe_ratios = {name: perf.sharpe_ratio for name, perf in strategy_performances.items()}
            risk_adjusted_ranking = sorted(sharpe_ratios.items(), key=lambda x: x[1], reverse=True)
            ...
            statistical_significance = {
                strategy: random.choice([True, False]) 
        ...
        try:
            with sqlite3.connect(self._db_path) as conn:
                conn.execute('''
                    ...
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                ''', (
                    performance.strategy_name, datetime.now().strftime('%Y-%m-%d'),
                    performance.period_start, performance.period_end, performance.total_return,
                    performance.annualized_return, performance.volatility, performance.sharpe_ratio,
                    performance.max_drawdown, performance.win_rate, performance.avg_win,
                    performance.avg_loss, performance.profit_factor, performance.total_trades,
                    performance.profitable_trades, performance.losing_trades,
                    performance.calmar_ratio, performance.sortino_ratio, performance.market_correlation,
                    performance.alpha, performance.beta, performance.information_ratio,
                    json.dumps(performance.monthly_returns), json.dumps(performance.quarterly_returns)
                ))
        ...
        try:
            with sqlite3.connect(self._db_path) as conn:
                conn.execute('''
                    ...
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                ''', (
                    comparison.comparison_date, comparison.period_days,
                    json.dumps(comparison.strategies), comparison.best_performer,
                    comparison.worst_performer, comparison.performance_spread,
    ...
    def export_comprehensive_report(self, file_path: str = "strategy_report.json"):
        ...
        try:
            ...
            for strategy in self._strategies:
                ...
                if performances:
                    strategy_performances[strategy] = [p.to_dict() for p in performances]
            ...
            report = {
                ...
                'summary': {
                    ...
                    'performance_spread': comparison.performance_spread if comparison else None
                }
            }
