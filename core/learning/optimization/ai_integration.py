...
class RestrictedUnpickler(pickle.Unpickler):
    ...
    def find_class(self, module: str, name: str):
        ...
        if (
            module_base not in self.ALLOWED_MODULES
...
def safe_pickle_load(file_path: str, max_size_mb: int = 100) -> Any:
    ...
    if file_size > max_size_bytes:
        raise ValueError(
            f"파일 크기({file_size / 1024 / 1024:.1f}MB)가 최대 허용 크기({max_size_mb}MB)를 초과합니다."
        )
...
from .genetic_optimizer import GeneticOptimizer, GeneticConfig  # noqa: E402
from .bayesian_optimizer import BayesianOptimizer, BayesianConfig  # noqa: E402
from .backtest_automation import BacktestEngine, ValidationSystem  # noqa: E402

# 모델 관련 import
try:
    from ..models.pattern_learner import PatternLearner, LearningConfig  # noqa: F401
    from ..models.prediction_engine import (
        PredictionEngine,
        PredictionConfig,
    )  # noqa: F401
    from ..models.feedback_system import FeedbackSystem  # noqa: F401
    from ..features.feature_selector import FeatureExtractor  # noqa: F401
    from ..analysis.daily_performance import DailyPerformanceAnalyzer  # noqa: F401
...
class ModelRegistry:
    ...
    def register_model(self, model_metadata: ModelMetadata) -> bool:
        ...
        try:
            ...
            if not os.path.exists(model_metadata.file_path):
                self._logger.error(
                    f"모델 파일을 찾을 수 없음: {model_metadata.file_path}",
                    exc_info=True,
                )
                return False
            ...
            self._logger.info(
                f"모델 등록 완료: {model_metadata.model_id} v{model_metadata.version}"
            )
            return True
    ...
    def get_models_by_type(self, model_type: str) -> List[ModelMetadata]:
        """타입별 모델 조회"""
        return [
            model for model in self._models.values() if model.model_type == model_type
        ]
    ...
    def _save_registry(self):
        ...
        try:
            ...
            registry_data = {"models": {}, "history": self._model_history}
        ...
        try:
            ...
            if os.path.exists(registry_file):
                ...
                for model_id, model_dict in registry_data.get("models", {}).items():
                    model_dict["training_date"] = datetime.fromisoformat(
                        model_dict["training_date"]
                    )
...
class IntegrationManager:
    ...
    def initialize_components(self) -> bool:
        ...
        try:
            ...
            if MODEL_IMPORTS_AVAILABLE:
                try:
                    ...
                    self._pattern_learner = get_pattern_learner(
                        feature_extractor, performance_analyzer
                    )
            ...
            if MODEL_IMPORTS_AVAILABLE:
                try:
                    ...
                    self._feedback_system = get_feedback_system(
                        self._prediction_engine, performance_analyzer
                    )
            ...
            successful_components = sum(
                1 for status in self._integration_status.values() if status
            )
            total_components = len(self._integration_status)

            self._is_integrated = (
                successful_components >= total_components * 0.7
            )  # 70% 이상 성공

            self._logger.info(
                f"컴포넌트 초기화 완료: {successful_components}/{total_components} 성공"
            )
            ...
            def predict_pattern(self, stock_code, date=None):
                return {
                    ...
                    "individual_predictions": {
                        "mock_model": np.random.uniform(0.5, 0.8)
                    },
                }
    ...
    def create_integrated_model(
        ...
        try:
            ...
            integrated_model = {
                ...
                "components": {
                    "parameter_manager": self._serialize_component(
                        self._parameter_manager
                    ),
                    "pattern_learner": self._serialize_component(self._pattern_learner),
                    "prediction_engine": self._serialize_component(
                ...
                "configuration": {
                    "genetic_config": (
                        asdict(GeneticConfig()) if self._genetic_optimizer else None
                    ),
                    "bayesian_config": (
                        asdict(BayesianConfig()) if self._bayesian_optimizer else None
                    ),
            ...
            model_metadata = ModelMetadata(
                ...
                performance_metrics={
                    "integration_score": sum(
                        1 for s in self._integration_status.values() if s
                    )
                    / len(self._integration_status),
    ...
    def deploy_integrated_model(
        self, model_metadata: ModelMetadata, config: DeploymentConfig = None
    ) -> DeploymentResult:
        """통합 모델 배포"""
        if config is None:
        ...
        deployment_id = (
            f"deploy_{model_metadata.model_id}_{datetime.now().strftime('%H%M%S')}"
        )
        ...
        try:
            ...
            # 상태 업데이트
            self._model_registry.update_model_status(
                model_metadata.model_id, ModelStatus.DEPLOYED
            )
    ...
    def _copy_model_to_deployment(
        self, model_metadata: ModelMetadata, config: DeploymentConfig
    ) -> Optional[str]:
        """모델을 배포 위치로 복사"""
        try:
            deployment_dir = os.path.join(
                self._data_dir, "deployed", config.environment
            )
            os.makedirs(deployment_dir, exist_ok=True)

            deployment_filename = (
                f"{model_metadata.model_type}_{config.model_version}.pkl"
            )
    ...
    def get_integration_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "integration_score": (
                sum(1 for s in self._integration_status.values() if s)
                / len(self._integration_status)
    ...
    def run_end_to_end_test(
        self, strategy_name: str = "momentum_strategy"
    ) -> Dict[str, Any]:
        """종단간 테스트 실행"""
        ...
        try:
            ...
            self._logger.info(
                f"종단간 테스트 완료 - 성공률: {test_results['success_rate']:.1%}"
            )
            return test_results
    ...
    def _test_parameter_manager(self, strategy_name: str) -> Dict[str, Any]:
        ...
        try:
            # 랜덤 파라미터 생성 테스트
            param_set = self._parameter_manager.create_random_parameter_set(
                strategy_name
            )
    ...
    def _test_model_integration(self, strategy_name: str) -> Dict[str, Any]:
        ...
        try:
            # 통합 모델 생성 테스트
            model_metadata = self.create_integrated_model(
                f"test_model_{datetime.now().strftime('%H%M%S')}", strategy_name
            )
...
def deploy_phase4_ai_system(strategy_name: str = "momentum_strategy") -> Dict[str, Any]:
    ...
    try:
        ...
        # 4. 통합 모델 생성
        model_metadata = integration_manager.create_integrated_model(
            "phase4_ai_system", strategy_name
        )
        if model_metadata:
            ...
            deployment_config = DeploymentConfig(
                model_version="1.0.0", environment="staging"
            )
            deploy_result = integration_manager.deploy_integrated_model(
                model_metadata, deployment_config
            )
            deployment_results["model_deployed"] = deploy_result.success
            deployment_results["deployment_status"] = (
                deploy_result.deployment_status.value
            )
        ...
        deployment_results["overall_success"] = (
            ...
            and deployment_results.get("model_created", False)
            and deployment_results.get("model_deployed", False)
        )

        deployment_results["end_time"] = datetime.now()
        deployment_results["total_duration"] = (
