...
class StockCode(BaseModel):
    ...
    def validate_code(cls, v: str) -> str:
        ...
        허용 형식:
        - 일반주: 6자리 숫자 (^\d{6}$)
        - 우선주: 4~5자리 숫자 + 알파벳 + 선택적 숫자 (^\d{4,5}[A-Z]\d?$)
        - 스팩: 5자리 숫자 + K (^\d{5}K$)
        """
        # 일반주: 6자리 숫자
        if re.match(r'^\d{6}$', v):
        ...
        # 우선주: 4~5자리 숫자 + 알파벳 + 선택적 숫자 (예: 37550L, 0037T0, 0072Z0, 0099X0)
        # \d? = 숫자 0개 또는 1개 (37550L도 허용)
        ...
        raise ValueError(
            f'종목코드 형식 오류: {v} '
            f'(일반주: 6자리 숫자, 우선주: 4~5자리숫자+알파벳+선택적숫자, 스팩: 5자리숫자+K)'
        )
