...
**Default Security List (hantu-server 용):**

| 포트 | 프로토콜 | 용도       | Source    | 비고                               |
| ---- | -------- | ---------- | --------- | ---------------------------------- |
| 22   | TCP      | SSH        | 0.0.0.0/0 | 서버 관리 접속                     |
| 8000 | TCP      | API Server | 0.0.0.0/0 | FastAPI 서버 (API_SERVER_KEY 인증) |

**삭제된 규칙 (마이그레이션 후 불필요):**

| 포트 | 프로토콜 | Source             | 삭제 이유                            |
| ---- | -------- | ------------------ | ------------------------------------ |
| 5432 | TCP      | 134.185.104.141/32 | All-in-One 통합으로 외부 접근 불필요 |

> **참고:**
>
> - PostgreSQL(5432)는 localhost에서만 접근 (외부 노출 안함)
> - 로컬 개발 시 SSH 터널(`./scripts/db-tunnel.sh`)로 접속
> - API 서버는 `API_SERVER_KEY` 환경변수로 인증
