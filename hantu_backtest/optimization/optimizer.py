...
class StrategyOptimizer:
    ...
    def __init__(
        ...
        # 결과 저장 경로
        self.results_dir = (
            Path(__file__).parent.parent / "data" / "results" / "optimization"
        )
    ...
    def optimize(self) -> Tuple[Dict[str, Any], pd.DataFrame]:
        ...
        try:
            ...
            logger.info(
                f"[optimize] 총 {len(param_combinations)}개 파라미터 조합 테스트"
            )
            ...
            best_params = {col: best_result[col] for col in self.param_grid.keys()}

            logger.info(f"[optimize] 최적 파라미터: {best_params}")
            logger.info(
                f"[optimize] 최적 성과: {self.metric}={best_result[self.metric]:.4f}"
            )
    ...
    def _save_results(self, results_df: pd.DataFrame, best_params: Dict[str, Any]):
        ...
        try:
            ...
            logger.info("[_save_results] 최적화 결과 저장 완료")
    ...
    def _plot_parameter_distributions(self, results_df: pd.DataFrame):
        ...
        try:
            ...
            fig, axes = plt.subplots(
                len(param_names), 1, figsize=(10, 5 * len(param_names))
            )
            ...
            logger.error(
                f"[_plot_parameter_distributions] 차트 생성 중 오류 발생: {str(e)}"
            )
    ...
    def _plot_parameter_correlations(self, results_df: pd.DataFrame):
        ...
        try:
            ...
            logger.error(
                f"[_plot_parameter_correlations] 차트 생성 중 오류 발생: {str(e)}"
            )
    ...
    def _plot_metric_relationships(self, results_df: pd.DataFrame):
        ...
        try:
            ...
            logger.error(
                f"[_plot_metric_relationships] 차트 생성 중 오류 발생: {str(e)}"
            )
