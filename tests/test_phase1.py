...
class TestWatchlistManager(unittest.TestCase):
    ...
    def test_add_stock(self):
        """종목 추가 테스트"""
        success = self.manager.add_stock(
            p_stock_code="005930",
            p_stock_name="삼성전자",
    ...
    def test_duplicate_add_stock(self):
        """중복 종목 추가 테스트"""
        # 첫 번째 추가
        success1 = self.manager.add_stock(
            p_stock_code="005930",
            p_stock_name="삼성전자",
        ...
        # 중복 추가 시도
        success2 = self.manager.add_stock(
            p_stock_code="005930",
            p_stock_name="삼성전자",
        ...
        with patch.object(self.screener, '_fetch_stock_data') as mock_fetch:
            ...
            for result in screening_results:
                if result["overall_passed"]:
                    success = self.manager.add_stock(
                        p_stock_code=result["stock_code"],
                        p_stock_name=result["stock_name"],
                        p_added_reason="스크리닝 통과",
                        p_target_price=70000,
                        p_stop_loss=50000,
                        p_sector=result["sector"],
                        p_screening_score=result["overall_score"]
                    )
                    self.assertTrue(success)
