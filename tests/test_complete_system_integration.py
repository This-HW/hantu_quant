...
def test_phase1_watchlist_system():
    ...
    try:
        ...
        screened_stocks = [
            ...
            and stock["per"] <= criteria["max_per"]
        ]
...
def test_integrated_workflow():
    ...
    try:
        ...
        # 스크리닝 기준 적용
        screened_stocks = [
            stock for stock in all_stocks if stock["market_cap"] >= 50000000000
        ]
        ...
        sorted_stocks = sorted(
            daily_analysis.items(), key=lambda x: x[1]["momentum_score"], reverse=True
        )
        selected_stocks = [stock[0] for stock in sorted_stocks[:3]]
        print(
            f"✅ 일일 선정 완료: {len(selected_stocks)}개 종목 ({', '.join(selected_stocks)})"
        )
        ...
        print(
            f"✅ 모니터링 활성화: {monitoring_status['monitored_stocks']}개 종목 감시 중"
        )
